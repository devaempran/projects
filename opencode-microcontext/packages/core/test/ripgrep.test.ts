...
describe("Ripgrep", () => {
  ...
  it.live("keeps node_modules out of glob results even with a broad positive pattern", () =>
    Effect.acquireUseRelease(
      Effect.promise(() => tmpdir()),
      (tmp) =>
        Effect.gen(function* () {
          ...
          // A broad positive override glob (unlike the bare "*" case above, which adds
          // no --glob at all) makes ripgrep stop honoring .gitignore for anything it
          // matches — "**" matches everything, so without an explicit node_modules
          // exclusion this would leak the whole ignored tree back into results.
          const files = yield* (yield* Ripgrep.Service).glob({ cwd: tmp.path, pattern: "**", limit: 100 })
          expect(files.map((item) => item.path)).toContain(RelativePath.make("src/index.js"))
          expect(files.map((item) => item.path)).not.toContain(RelativePath.make("node_modules/pkg/index.js"))
        }),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
