...
import { SessionOrchestrator } from "../orchestrator/entry"
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const runTurnAttempt = Effect.fn("SessionRunner.runTurn")(function* (
      ...
      const orchestratorConfig = Config.latest(yield* config.entries(), "experimental")?.orchestrator
      if (
        toolMaterialization &&
        SessionOrchestrator.shouldOrchestrate({
          enabled: orchestratorConfig?.enabled,
          agentOrchestrator: agent.info?.orchestrator,
        })
      ) {
        ...
        const emitOrchestrator = (
          ...
          DateTime.now.pipe(
            Effect.flatMap((timestamp) => publish({ sessionID: session.id, timestamp })),
            Effect.asVoid,
          )
        ...
        const orchestratorObserver: SessionOrchestrator.OrchestratorObserver.Interface = {
          planStarted: (data) =>
        ...
        const emitText = (text: string) =>
          Effect.gen(function* () {
            const id = `${assistantMessageID}_txt`
            yield* publish(LLMEvent.stepStart({ index: 0 }))
            yield* publish(LLMEvent.textStart({ id }))
            yield* publish(LLMEvent.textDelta({ id, text }))
            yield* publish(LLMEvent.textEnd({ id }))
            yield* publish(LLMEvent.stepFinish({ index: 0, reason: "stop" }))
            yield* publish(LLMEvent.finish({ reason: "stop" }))
          }).pipe(Effect.orDie)
        yield* SessionOrchestrator.runLive({
          sessionID: session.id,
          agent: agent.id,
          model,
          prompt: SessionOrchestrator.latestUserText(context) ?? "",
          ...
          // (llm acquired above, OrchestratorState.node is a layer dep) so discharge them here to keep
          // runTurnAttempt's requirement channel empty, matching the RunTurn signature.
