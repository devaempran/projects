...
import type { OrchestratorSchema } from "./orchestrator/schema"
...
export const OrchestratorStateTable = sqliteTable("orchestrator_state", {
  session_id: text()
    .$type<SessionSchema.ID>()
    .primaryKey()
    .references(() => SessionTable.id, { onDelete: "cascade" }),
  status: text().$type<OrchestratorSchema.Status>().notNull(),
  iteration: integer().notNull().default(0),
  max_iterations: integer().notNull(),
  data: text({ mode: "json" }).notNull().$type<OrchestratorSchema.Data>(),
  ...Timestamps,
})

export const OrchestratorMemoryTable = sqliteTable(
  "orchestrator_memory",
  {
    session_id: text()
      .$type<SessionSchema.ID>()
