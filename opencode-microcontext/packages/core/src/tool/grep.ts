...
export const toModelOutput = (output: ModelOutput, truncated?: boolean) => {
  ...
  if (truncated) {
    lines.push("")
    lines.push(`(Results are truncated: showing first ${output.length} matches. Consider using a more specific pattern, path, or include filter.)`)
  }
  return lines.join("\n")
}
            ...
            Effect.gen(function* () {
              ...
              return yield* ripgrep
                .grep({
                  ...
                  include: input.include,
                  // See glob.ts: an omitted `limit` must not mean unbounded — a broad
                  // pattern over a large tree can return enough matches to blow a
                  // small model's context window in one tool result.
