...
// The V1 loader rejects unknown top-level keys to catch typos, but config files may now use the V2 shape
// (e.g. `agents`, `orchestrator`). Allow every V2 top-level key through the gate; the V1 decode drops them
// and the V2 core Config service, reading the same file, consumes them. Derived from the schema to avoid drift.
const V2_TOP_LEVEL_KEYS: ReadonlySet<string> = new Set(Object.keys(ConfigV2.Info.fields))
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      const data = ConfigParse.schema(ConfigV1.Info, normalizeLoadedConfig(parsed), source, V2_TOP_LEVEL_KEYS)
    ...
    const updateGlobal = Effect.fn("Config.updateGlobal")(function* (config: Info) {
      ...
      if (!file.endsWith(".jsonc")) {
        const existing = ConfigParse.schema(ConfigV1.Info, ConfigParse.jsonc(before, file), file, V2_TOP_LEVEL_KEYS)
        ...
        next = ConfigParse.schema(ConfigV1.Info, ConfigParse.jsonc(updated, file), file, V2_TOP_LEVEL_KEYS)
