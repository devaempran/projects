...
export function schema<S extends EffectSchema.Decoder<unknown, never>>(
  ...
  data: unknown,
  source: string,
  // Top-level keys to accept even though they are absent from `schema`. Used so the V1 loader tolerates
  // V2-shaped config files (e.g. `agents`): the keys pass this strict gate, get dropped by the V1 decode,
  // and are consumed instead by the V2 core Config service that reads the same file.
  allowExtraKeys?: ReadonlySet<string>,
): DeepMutable<S["Type"]> {
  const extra = topLevelExtraKeys(schema, data, allowExtraKeys)
  if (extra.length) {
...
function topLevelExtraKeys(schema: EffectSchema.Top, data: unknown, allowExtraKeys?: ReadonlySet<string>) {
  ...
  return Object.keys(data).filter((key) => !known.has(key) && !allowExtraKeys?.has(key))
}
