...
import { OrchestratorUiRoute } from "@opencode-ai/server/orchestrator-ui"
...
// Live (non-durable) view of the small-context ReAct orchestrator flow — a raw route
// outside the typed API surface, same as docRoute above. Reused as-is from packages/server
// (the "v2" server), which doesn't compose into this instance route tree by default.
const orchestratorUiRoute = OrchestratorUiRoute.pipe(Layer.provide(authOnlyRouterLayer))
